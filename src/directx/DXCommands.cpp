...
__CRT_UUID_DECL(ID3D12QueryHeap,
    0x0d9658ae, 0xed45, 0x469e, 0xa6, 0x1d, 0x97, 0x0e, 0xd6, 0x5d, 0x7e, 0x8e);
#endif
...
namespace vireo {
    ...
    void DXCommandList::convertState(
        ...
        if (oldState == ResourceState::UNDEFINED && newState == ResourceState::COMPUTE_WRITE) {
            ...
            srcState = D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE;
            dstState = D3D12_RESOURCE_STATE_RENDER_TARGET;
        } else if (oldState == ResourceState::RENDER_TARGET_COLOR && newState == ResourceState::COMPUTE_WRITE) {
            srcState = D3D12_RESOURCE_STATE_RENDER_TARGET;
            dstState = D3D12_RESOURCE_STATE_UNORDERED_ACCESS;
            ...
            srcState = D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE | D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE;
            dstState = D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE;
        } else if (oldState == ResourceState::COMPUTE_WRITE && newState == ResourceState::RENDER_TARGET_COLOR) {
            srcState = D3D12_RESOURCE_STATE_UNORDERED_ACCESS;
            dstState = D3D12_RESOURCE_STATE_RENDER_TARGET;
        } else {
    ...
    void DXCommandList::drawIndirect(
        ...
        commandList->ExecuteIndirect(
            drawIndirectCommandSignatures.at(currentlyBoundPipeline).at(stride).Get(),
            drawCount,
    ...
    void DXCommandList::checkIndirectCommandSignature(
        ...
        assert(currentlyBoundPipeline != nullptr);
        if (!drawIndirectCommandSignatures.contains(currentlyBoundPipeline)) {
            drawIndirectCommandSignatures[currentlyBoundPipeline] = std::unordered_map<uint32_t, ComPtr<ID3D12CommandSignature>>();
        }
        if  (!drawIndirectCommandSignatures[currentlyBoundPipeline].contains(stride)) {
            ...
            if (commandStride != stride) {
                ...
                const auto resources =
                    std::static_pointer_cast<DXPipelineResources>(currentlyBoundPipeline->getResources());

                D3D12_INDIRECT_ARGUMENT_DESC args[2] = {};
                args[0].Type = D3D12_INDIRECT_ARGUMENT_TYPE_CONSTANT;
                args[0].Constant.RootParameterIndex = resources->getPushConstantsRootParameterIndex();
            ...
            drawIndirectCommandSignatures[currentlyBoundPipeline][stride] = commandSignature;
        }
    }
